...
export function AdminImportExport() {
  ...
  return (
    <section>
      <h1 className="mb-2 text-3xl font-black">استيراد وتصدير Excel</h1>
      <p className="mb-6 text-ink/60">يدعم الأعمدة العربية والإنجليزية، وإنشاء الأقسام تلقائيًا عند عدم وجودها.</p>
      <div className="grid gap-5 lg:grid-cols-2">
        <form className="admin-card grid gap-3" onSubmit={submit}>
          <h2 className="text-xl font-black">استيراد المنتجات</h2>
          <input className="input" type="file" accept=".xlsx,.xls" onChange={(event) => setFile(event.target.files?.[0] || null)} />
        ...
        <div className="admin-card">
          ...
          <div className="flex flex-wrap gap-3">
            <button className="btn-secondary" onClick={() => downloadAdminFile("/admin/excel/template", "ghaleb-mall-products-template.xlsx")}>تحميل قالب Excel</button>
            <button className="btn-secondary" onClick={() => downloadAdminFile("/admin/export/products/excel", "ghaleb-mall-products.xlsx")}>تصدير المنتجات</button>
            <button className="btn-secondary" onClick={() => downloadAdminFile("/admin/orders/export/excel", "ghaleb-mall-orders.xlsx")}>تصدير الطلبات</button>
          </div>
        </div>
      </div>
      {result && (
